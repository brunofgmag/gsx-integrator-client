...
namespace
{
    ...
    bool IsGsxBoardingDone(const TurnaroundContext& ctx, const GsxStateStatus boardingState)
    {
        return boardingState == GsxStateStatus::Completed
            || ctx.gsxGateway->WasStateCompleted(GsxState::Boarding);
    }

    bool HasGivenUpOnTheLoader(const TurnaroundContext& ctx)
    {
        return ctx.data.loaderAwaitingDoor != CargoLoader::None
            && ctx.data.loaderDoorWaitTicks >= kLoaderDoorGiveUpTicks;
    }

    bool HasGivenUpOnTheCargoFlag(const TurnaroundContext& ctx)
    {
    ...
    bool IsBarFull(const TurnaroundContext& ctx)
    {
    ...
    bool IsCargoHeldBehindTheStairs(const TurnaroundContext& ctx)
    {
    ...
    void NoteLoaderAwaitingDoor(TurnaroundContext& ctx)
    {
        auto& data = ctx.data;
    ...
    void NoteCargoFlagAfterTheService(TurnaroundContext& ctx, const bool serviceClosed)
    {
        auto& data = ctx.data;
    ...
    void FinishBoarding(TurnaroundContext& ctx)
    {
        auto& data = ctx.data;
        data.boardedPassengers = data.plannedPassengers;
    ...
    void AdvanceBoardingBar(TurnaroundContext& ctx)
    {
        auto& data = ctx.data;
...
bool BoardingTrack::IsInterrupted(const TurnaroundContext& ctx)
{
    const GsxStateStatus boardingState = BoardingStatus(ctx);

    return ctx.data.boardingBaselined
        && !IsGsxBoardingDone(ctx, boardingState)
        && boardingState < GsxStateStatus::Requested;
}

bool BoardingTrack::Advance(TurnaroundContext& ctx)
{
    auto& data = ctx.data;

    const GsxStateStatus boardingState = BoardingStatus(ctx);
    const bool isCompleted = IsGsxBoardingDone(ctx, boardingState);
