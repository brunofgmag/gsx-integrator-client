...
- FSS Embraer E190/E195: the Call RAMP button on the audio control panel. Press it and let go; the button itself pops back up on its own, and the client clears the call.
...
- On the E190/E195, like the 727, the client loads from its own OFP and never reads the EFB. Do not import a SimBrief plan into the EFB: it overwrites the fuel and payload the client just loaded the moment boarding finishes, undoing what you dispatched. The EFB's own GSX remote control also opens the GSX menu by itself while the client is driving the turnaround; if you see it doing that, turn `enableGsxRemoteControl` off in the EFB settings.
